...
<style>
    ...
    .bg-pastel-info { background-color: #eff6ff; border-radius: 12px; }
    .text-pastel-info { color: #2563eb; }
</style>
    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h3 class="mb-0 text-dark fw-bold">Lapor Progres Lomba Baru</h3>
                <a href="{{ route('user.submissions.index') }}" class="btn btn-outline-pastel">
                    Kembali
                </a>
            </div>
            ...
            <div class="card card-pastel bg-white">
                <div class="card-body p-5">
                    ...
                    <form action="{{ route('user.submissions.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf

                        <div class="mb-4">
                            <label for="competition_id" class="form-label-pastel">Pilih Info Lomba <span class="text-danger">*</span></label>
                            <select class="form-select form-select-pastel @error('competition_id') is-invalid @enderror" id="competition_id" name="competition_id" required>
                                <option value="" class="text-muted">-- Lomba yang Diikuti --</option>
                        ...
                        <div class="mb-5">
                            <label for="proposal_path" class="form-label-pastel">Upload Karya / Proposal (PDF) <span class="text-danger">*</span></label>
                            <input type="file" class="form-control form-control-pastel @error('proposal_path') is-invalid @enderror" 
                                   id="proposal_path" name="proposal_path" accept="application/pdf" required>
                            <div class="form-text text-muted mt-2">Format PDF (Maks 100MB). Karya ini diupload sebagai Cek Arsip Jejak Karya.</div>
                            @error('proposal_path')
                                <div class="invalid-feedback mt-2">{{ $message }}</div>
                            @enderror
                        </div>

                        <div class="bg-pastel-info text-pastel-info p-3 mb-4">
                            <span class="small fw-medium">Info: Sertifikat bisa kamu upload nanti pas update progres karya.</span>
                        </div>

                        <div class="d-grid gap-2 d-md-flex justify-content-md-end mt-4">
                            <button type="submit" class="btn btn-pastel">Simpan Karya</button>
                        </div>
                    </form>
